...
<main class="flex-1 ms-10 me-10">
    ...
    <form action="{{route('create-player.store')}}" method="POST" enctype="multipart/form-data" class="max-w-xl mx-auto">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="first_name" name="first_name" class="mt-2 p-2 w-full border @error('first_name') border-red-500 @else border-[var(--azul)] @enderror rounded bg-[var(--crema)]" required placeholder="Escribe el nombre" value="{{ old('first_name') }}">
            @error('first_name')
        ...
        <div class="mb-4">
            ...
            <input type="text" id="last_name" name="last_name" class="mt-2 p-2 w-full border @error('last_name') border-red-500 @else border-[var(--azul)] @enderror rounded bg-[var(--crema)]" required placeholder="Escribe los apellidos" value="{{ old('last_name') }}">
            @error('last_name')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="email" id="email" name="email" class="mt-2 p-2 w-full border @error('email') border-red-500 @else border-[var(--azul)] @enderror rounded bg-[var(--crema)]" placeholder="ejemplo@correo.com" value="{{ old('email') }}">
            @error('email')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select id="partner" name="partner" class="mt-2 p-2 w-full border @error('partner') border-red-500 @else border-[var(--azul)] @enderror rounded bg-[var(--crema)]" required>
                <option value="1" {{ old('partner') == 1 ? 'selected' : '' }}>Sí</option>
                <option value="0" {{ old('partner') == 0 ? 'selected' : '' }}>No</option>
            </select>
            @error('partner')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
